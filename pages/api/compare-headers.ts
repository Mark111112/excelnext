...
import { compareHeaders, getUploadedFiles } from '@/utils/excelUtils';
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = compareHeaders(files, sheetIndexNum, headerRowNum);
    ...
    return res.status(500).json({
      success: false,
      message: 'Failed to compare headers',
